...
If true, the cache would only bust once per memory write. However, to get to 0 cache busts per write, we will need to remove BOTH  
memory rewrite opportunities as the rest of the plan lays out.
